...
```
  -add <url>             Add <url> to the playlist of an existing cxine, then exit.
  ...
  -win     <win id>      'root', 'fullscreen', 'sticky', 'shaded', 'ontop', 'stickontop' or id of window to reparent into.
  ...
  -stop_xscreensaver     Disable screensaver during playing, re-enable it on exit.
  ...
  -keygrab               Register keygrabs, a comma-separated list of keys. See keygrabs below.
...
The `-win` command supports a number of window types. `-win root` means draw on the root desktop window. `-win fullscreen` is fullscreen mode. `-win ontop` creates a window that always stays above other windows. `-win shaded` creates a window that is initially shaded (just the title-bar visible, if the window-manager supports this). `-win sticky` causes the window to appear on all desktops, giving the suggestion that its stuck to the screen. However, the window created with `-win sticky` still obeys stacking order, and doesn`t stay on top of other windows, breaking the illusion of being stuck to the glass. `-win stickontop` creates a fully sticky window that appears on all desktops and stays above other windows.

The other use of `-win` is to specify a window to embed/reparent into. The window id can be specified either in decimal, or in hex like `-win 0x30001` 
...
Bookmark playback can be disabled with `-startms 0` option, which tells cxine to start 0 milliseconds into the media (i.e. at the beginning).
